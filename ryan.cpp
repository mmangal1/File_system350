...
void ryan::inode::initialize(string filename, ryan *disk){
...
void ryan::create(char* file_name, int num_blocks, int block_size){
	/* Creates disk */
...
void ryan::write_inode_map(int inode_map[], char* file_name, int block_size, int num_blocks, FILE *fp){
	uint8_t currbyte = 0;
	int bitcount = 0;
	int totalcount = 0;
	inode_map[60] = 0;
...
void ryan::write_fbl(int free_block_list[], char* file_name, int block_size, int num_blocks, FILE *fp){
	int currbyte = 0;
	int bitcount = 0;
	int totalcount = 0;
...
void ryan::write_sb(int offset, int block_size, int num_blocks, FILE *fp){
	fseek(fp, 0, SEEK_SET);
	fprintf(fp, "%d%d%d", num_blocks, block_size, offset);
}

void ryan::write_inode_to_disk(int offset, FILE *fp, inode *node){
...
void ryan::read_inode_from_disk(int offset, FILE *fp){
	fseek(fp, offset, SEEK_SET);
...
void ryan::import(char* ssfs_filename, char* unix_filename){}
void ryan::cat(char* filename){}
void ryan::del(char* filename){}
void ryan::write_to_disk(string filename){
...
void ryan::read(char *filename, int startByte, int numByte){
...
int ryan::get_block_size(){
	return sb.block_size;
}

int ryan::get_num_blocks(){
	return sb.num_blocks;
}

void ryan::update_inode_map(int index){
	inode_map[index] = 1;
}

int ryan::read_free_disk_iMap(){
	int bit_index = 0;
	char buff[32];
...
/*void ryan::update_inode(char* filename, int index){
